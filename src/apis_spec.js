...
describe('apis', function () {
    ...
    describe('hashKey', function () {
        ...
        it('is the same hash key that is generated when used multiple times', function () {
            var obj = {};
            expect(hashKey(obj)).toEqual(hashKey(obj));
        });

        it('does not change hash key when the object value changes', function () {
            var obj = {a: 42};
        ...
        it('is not the same hash key for different objects with same value', function () {
            var obj1 = {a: 42};
            var obj2 = {a: 42};
            expect(hashKey(obj1)).not.toEqual(hashKey(obj2));
        });

        it('is function:[unique key] for functions', function () {
            var fn = function (a) {
                return a;
            };
        ...
        it('is the same key when asked for the same function multiple times', function () {
            var fn = function () {};
            expect(hashKey(fn)).toEqual(hashKey(fn));
        });

        it('is not the same hash key even if two functions are identical', function () {
            var fn1 = function () {
                return 42;
            };
            var fn2 = function() {
        ...
        it('uses $$hashKey if already assigned', function () {
    ...
    describe('HashMap', function () {
        ...
        it('returns the value of the removed key', function () {
            var map = new HashMap();
            map.put(42, 'fourty two');
            var value = map.remove(42);
            expect(value).toBe('fourty two');
        });
